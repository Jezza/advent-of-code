...
fn main() {
    ...
    aoc(part_one, vec![(TEST_1, 95437), (INPUT, 1325919)]);
    aoc(part_two, vec![(TEST_1, 24933642), (INPUT, 2050735)]);
}
...
fn handle_input<'a>(input: &'a str) -> impl Iterator<Item = Node> + 'a {
    let current_depth = input
        .lines()
        .skip(1)
        .filter_map(|line| line.strip_prefix("$ cd "))
        .fold(2, |acc, item| if item == ".." { acc - 1 } else { acc + 1 });

    let mut stack = vec![0; current_depth];

    input.lines().rev().filter_map(move |line| {
...
fn part_two(input: &str) -> u64 {
    ...
    for node in handle_input(input) {
        match node {
            ...
            Node::Folder(size) => folders.push(size),
        }
    }

    folders.sort_unstable();
    let index = folders
        .binary_search_by(|folder| (free_space + *folder).cmp(&30_000_000))
        .unwrap_err();
