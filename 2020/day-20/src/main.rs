...
fn parse_input() -> (Vec<Tile>, HashMap<u16, Vec<TileId>>) {
    ...
    tiles
        ...
        .flat_map(|tile| {
            ...
            std::array::IntoIter::new(tile.grid.as_symmetries()).map(move |sym| (id, sym))
        })
        .for_each(|(id, sym)| lookup.entry(sym).or_insert_with(Vec::new).push(id));
...
fn part_two() -> u64 {
    ...
    let adjacency_map = tiles
        .iter()
        .map(|tile| {
            let adjacent = lookup
                .iter()
                .filter(|&(_, ids)| ids.contains(&tile.id))
                .map(|(_, ids)| ids.iter().find(|&id| *id != tile.id).copied().unwrap())
                .collect::<HashSet<_>>();
    ...
    let first_corner_piece = adjacency_map
        .iter()
    ...
    fn add(left: Point, right: Point) -> Option<Point> {
        ...
        (point.0 >= 0 && point.1 >= 0).then_some(point)
    }

    const OFFSETS: [(Point, Direction); 2] =
        [((-1, 0), Direction::North), ((0, -1), Direction::West)];

    let mut remove_tile = move |id: TileId| {
        let index = tiles.iter().position(|tile| tile.id == id).unwrap();
    ...
    fn transform_grid(grid: &mut Grid, orientation: &(usize, Direction)) {
        match orientation {
            ...
            (1, Direction::North) | (4, Direction::West) => grid.mirror_horizontal(),
    ...
    {
        let _corner = grid_map.get_mut(&(0, 0)).unwrap();
    ...
    for point in enumerate_points(length).into_iter().skip(2) {
        let syms = grid_map.get(&point).unwrap().grid.as_symmetries();

        let orientations = OFFSETS
            .iter()
    ...
    for x in 0usize..length as usize {
        for y in 0usize..length as usize {
            ...
            let inner_grid = tile
                ...
                .iter()
                .filter(|&point| {
                    point.0 != 0 && point.0 != size - 1 && point.1 != 0 && point.1 != size - 1
                })
...
#    ##    ##    ###
 #  #  #  #  #  #
    ...
    let monster_width = monster.lines().map(|line| line.len()).max().unwrap();
