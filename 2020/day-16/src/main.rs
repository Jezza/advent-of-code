...
fn part_one() -> usize {
    let mut it = include_str!("../input/input.txt").split("\n\n");
    ...
    nearby_tickets
        ...
        .filter_map(|ticket| {
            ticket.iter().find(|v| {
                rules
                    .iter()
                    .all(|(_, lower, upper)| !(lower.contains(v) || upper.contains(v)))
            })
        })
...
fn part_two() -> usize {
    ...
    let mut nearby_tickets = nearby_tickets
        .lines()
        .skip(1)
        .map(|ticket| parse_ticket(ticket))
        .filter(|ticket| {
            ticket.iter().all(|v| {
                rules
                    .iter()
                    .any(|(_, lower, upper)| lower.contains(v) || upper.contains(v))
            })
        })
        .collect::<Tickets>();
    ...
    let mut all_possible_rules = (0..cols)
        .map(|col| {
            rules
                .iter()
                .enumerate()
