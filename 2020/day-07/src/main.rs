...
fn contains(graph: &Graph, key: &str) -> bool {
    key == "shiny gold" || graph[key].iter().any(|(_, k)| contains(graph, k))
}

#[cfg(feature = "cached")]
fn contains_cached(
    cache: &mut HashMap<&'static str, bool>,
    graph: &Graph,
    key: &'static str,
) -> bool {
    if !cache.contains_key(key) {
        let result = graph[key]
            .iter()
            .any(|(_, k)| contains_cached(cache, graph, k));
...
fn part_one() -> usize {
    ...
    graph.keys().filter(|k| contains(graph, k)).count() - 1 // remove one, as when we hit shiny-gold, we count that as a bag that can hold itself.
}
...
fn part_one() -> usize {
    ...
    graph
        ...
        .count()
        - 1 // remove one, as when we hit shiny-gold, we count that as a bag that can hold itself.
}
...
fn parse_line(input: &str) -> (&str, Vec<(usize, &str)>) {
    let (key, contents) = input.split_once(" bags contain ").unwrap();

    if contents == "no other bags." {
        return (key, vec![]);
    }

    let items: Vec<(usize, &str)> = contents
        .split(", ")
        .map(|item| {
