...
fn main() {
    ...
    aoc(
        part_one,
        vec![(INPUT, 69914999975369), (LE_DIRK, 99911993949684)],
    );
    aoc(
        part_two,
        vec![(INPUT, 14911675311114), (LE_DIRK, 62911941716111)],
    );
}
...
fn run_program(operations: &[Op], input: Word) -> bool {
    ...
    for op in operations {
        ...
        match *op {
            Input(reg) => {
                state[reg as usize] = match input.next() {
                    Some(value) => value,
                    None => {
            ...
            Op::Add(a, b) => {
                state[a as usize] = state[a as usize]
                    + match b {
...
fn validate_model(
    ...
    for step in 0..14 {
        let x = stack.last().copied().unwrap_or_default() + checks[step] as Word;
...
fn parse_input(input: &str) -> (Vec<bool>, Vec<i8>, Vec<i8>) {
    let validation_program: Vec<Op> = input
        .lines()
        .map(|line| {
    ...
    validation_program
        .split(|op| matches!(op, Op::Input(_)))
