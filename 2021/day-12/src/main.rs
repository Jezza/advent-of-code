...
fn main() {
    ...
    aoc(
        part_one,
        vec![(TEST_1, 10), (TEST_2, 19), (TEST_3, 226), (INPUT, 3779)],
    );
    aoc(
        part_two,
        vec![(TEST_1, 36), (TEST_2, 103), (TEST_3, 3509), (INPUT, 96988)],
    );
}
...
fn parse_input(input: &str) -> Vec<Node> {
    ...
    macro_rules! find_or_insert {
        ($name:ident) => {{
            nodes.iter().position(|t| t.2 == $name).unwrap_or_else(|| {
...
fn dfs(nodes: &[Node], revisit: bool) -> usize {
    ...
    let start_node = nodes.iter().position(|t| t.2 == "start").unwrap();
    let end_node = nodes.iter().position(|t| t.2 == "end").unwrap();
