...
impl Board {
	...
	fn cleanup(&mut self) {
		self.positions.iter_pos_tuples()
			.for_each(|pos @ (x, y)| {
	...
	fn possible_moves(&self) -> Vec<(Board, u64)> {
		...
		positions.iter_pos_tuples()
			.flat_map(|pos @ (x, y)| {
				...
				if y == 0 {
					let target @ (target_x, target_y) = (ty_offset * 2 + 3, remaining[ty_offset] as usize);
	...
	fn has_path(&self, start @ (start_x, start_y): Position, target @ (target_x, target_y): Position) -> bool {
		if start == target {
			true
		} else if start_y == 0 {
			// Starts in a hallway.
			...
			// Check if only self is in the given range.
			only_self && (start_x.min(target_x)..=start_x.max(target_x))
				.filter(|x| *self.positions.get(*x as usize, 0) != b'.')
				.count() == 0
		} else {
...
fn search(board: Board) -> u64 {
	dijkstra(
		...
		|board| board.remaining == [0; 4],
	).expect("Ya dun goof'd").1
}
