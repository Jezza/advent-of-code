...
fn explode(symbols: &mut Vec<Symbol>) -> bool {
	...
	symbols[..start].iter_mut()
		.rev()
		.find_map(|item| if let Symbol::Value(value) = item { Some(value) } else { None })
		.if_present(|mut previous| *previous += left);

	symbols[end..].iter_mut()
		.find_map(|item| if let Symbol::Value(value) = item { Some(value) } else { None })
		.if_present(|mut next| *next += right);
...
fn split(symbols: &mut Vec<Symbol>) -> bool {
	let matched_symbol = symbols.iter()
		.enumerate()
		.find_map(|(pos, symbol)| {
