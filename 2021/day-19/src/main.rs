...
fn try_align(aligned: &Scanner, candidate: &Scanner) -> Option<(Scanner, Beacon)> {
	...
	'dimension:
	for dim in 0..3 {
		let dim_values: Vec<_> = aligned.iter()
			.map(|beacon| beacon[dim])
			.collect();
		...
		for (dim, mirror) in [(0, 1), (1, 1), (2, 1), (0, -1), (1, -1), (2, -1)] {
			...
			let transforms: Vec<_> = candidate.iter()
				.map(|beacon| beacon[dim] * mirror)
				.collect();

			let frequencies: HashMap<Coord, u16> = dim_values.iter().cartesian_product(transforms.iter())
				.map(|(a, b)| *b - *a)
			...
			let (value, count) = frequencies.iter()
				.max_by_key(|(diff, count)| **count)
				.unwrap();
			...
			if *count >= 12 {
				...
				let offsets: Vec<_> = transforms.into_iter()
					.map(|v| v - value)
					.collect();
				result.push(offsets);
				delta.push(value);
...
fn part_two(input: &str) -> u64 {
	...
	shifts.iter().cartesian_product(shifts.iter())
		.map(|(left, right)| {
