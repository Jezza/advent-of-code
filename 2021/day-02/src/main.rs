...
fn main() {
    ...
    aoc(part_one_unified, vec![(TEST_1, 150), (INPUT, 2070300)]);
    aoc(part_two_unified, vec![(TEST_1, 900), (INPUT, 2078985210)]);
}
...
fn part_two(input: &str) -> u32 {
    let (_, x, y) = input.lines().filter_map(Command::try_from).fold(
        (0, 0, 0),
        |(mut aim, mut x, mut y), command| {
            match command {
