...
fn fold(input: &str, n: usize) -> Points {
	...
	let mut points: Points = points.lines()
	...
	for line in instructions.lines().take(n) {
		...
		if instruction.ends_with("x") {
			points.iter_mut().filter(|p| p.0 > fold).for_each(|p| p.0 = 2 * fold - p.0);
		} else {
			points.iter_mut().filter(|p| p.1 > fold).for_each(|p| p.1 = 2 * fold - p.1);
		}
	}
...
fn display_points(points: Points) -> String {
	let (width, height) = points.iter()
		.fold((0, 0), |acc, item| (acc.0.max(item.0 + 2), acc.1.max(item.1 + 1)));

	let mut grid = vec![b' '; (width * height) as usize];
	points.into_iter().for_each(|(x, y)| grid[(x + y * width) as usize] = b'#');
