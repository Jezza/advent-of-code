...
fn main() {
    ...
    aoc(part_one, vec![(TEST_1, 198), (INPUT, 4191876)]);
    aoc(part_two, vec![(TEST_1, 230), (INPUT, 3414905)]);
    ...
    aoc(
        ...
        vec![(test_1.clone(), 198), (input.clone(), 4191876)],
    );
    aoc(
        part_two_existing_input,
        vec![(test_1, 230), (input, 3414905)],
    );
}
...
fn read_input(input: &str) -> (Vec<u16>, u16, u16) {
    ...
    let bits = input.lines().map(|line| line.len()).max().unwrap() as u16;
...
fn generate_report_bit(bit: u16, input: &[u16]) -> bool {
    let (zeros, ones) =
        input
            .iter()
