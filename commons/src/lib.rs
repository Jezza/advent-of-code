...
pub fn aoc<I, O, F, P>(handler: F, it: P)
where
    I: Clone,
    F: Fn(I) -> O,
    ...
    for item in it {
        ...
        if measure {
            ...
            println!(
                "\t{:>22}\tbench:\t{:>11} ns/iter (+/- {})",
                expected,
                dumb_thousands(median),
                dumb_thousands(deviation)
            );
        } else {
            println!("\t{:>22}", expected);
        }
...
pub mod grid {
        ...
        [T; W * H]: Sized,
    {
        ...
        pub fn iter_pos_tuples_rev(&self) -> impl Iterator<Item = (usize, usize)> {
            ...
            (0..(self.width * self.height)).rev().map(move |i| {
                let x = i % width;
                let y = i / width;
        ...
        H: FnMut(&mut G, usize, usize, &'a str),
    {
        let (width, mut height) = row_splitter(input)
            ...
            .fold(
                (usize::MIN, usize::MIN),
                |(width, height), (index, value)| (width.max(value), height.max(index)),
            );
